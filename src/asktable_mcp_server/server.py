...
default_datasource_id = config['datasource_id']  # 改名以区分
...
async def get_sql(query: str) -> str:
    ...
    message = await get_asktable_sql(default_datasource_id, query)
    return message
...
async def get_datasouce_data(query: str) -> str:
    ...
    """
    # ds_id = datasource_id or default_datasource_id
    message = await get_asktable_data(default_datasource_id, query)
    return message
